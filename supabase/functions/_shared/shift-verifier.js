...
const espressoStations = ["grinder", "espresso", "steam", "coldBrew"];
const coldDrinkStations = ["ice", "sparkling", "blender"];
...
const takeFridgeIngredient = (state, itemId) =>
  state.inventory.length >= inventoryLimit
    ? {
        ...state,
        notice: "작업대가 가득 찼습니다",
...
const preferCurrentOrderPath = (candidates, target, recipeBook) =>
...
const serve = (state, uidToServe) => {
  ...
  const fever =
    combo >= Math.max(3, 5 - Math.floor(state.upgrades.feverCharge / 2)) && !state.fever
      ? 15 + state.upgrades.feverDuration * 3
      : state.fever;
...
const applyShiftAction = (state, action, recipeBook = recipes) => {
  ...
  switch (action.kind) {
    case "station": {
      ...
      if (stationUnlockStage[station] > state.stageId || station === "fridge" || station === "water")
        throw new Error("STATION_LOCKED");
    ...
    case "fridge":
      if (
        !fridgeIngredients.some(
          ({ itemId, minStage }) => itemId === action.target && minStage <= state.stageId,
        )
      )
        throw new Error("INGREDIENT_LOCKED");
...
const verifyShift = (session, receipt, wallSeconds) => {
  ...
  if (
    !Number.isInteger(receipt.elapsed) ||
    receipt.elapsed < 0 ||
    receipt.elapsed > 360 ||
    receipt.elapsed > wallSeconds + 2
  )
    throw new Error("INVALID_DURATION");
  if (!Array.isArray(receipt.actions) || receipt.actions.length > 12e3) throw new Error("INVALID_ACTIONS");
  if (
    !Array.isArray(receipt.seenMenuStages) ||
    receipt.seenMenuStages.length > 15 ||
    receipt.seenMenuStages.some((stage) => !Number.isInteger(stage) || stage < 1 || stage > session.stageId)
  )
    throw new Error("INVALID_MENU_STAGES");
