  ...
  try {
      ...
      result = verifyShift(
        {
          ...
          seed: session.seed,
          upgrades: { ...defaultUpgrades, ...session.upgrades },
        },
        receipt,
        (Date.now() - Date.parse(session.started_at)) / 1000,
      );
    } catch {
      return json({ error: "INVALID_SHIFT_RECORD" }, 422);
    }
    ...
    const { error: commitError } = await admin.rpc("commit_verified_shift", {
      ...
      p_result: {
        ...
        gold: state.gold,
        score: score.total,
        completedOrders: state.orderSequence,
        mistakes: state.mistakes,
        discardedItems: state.discardedItems,
        maxCombo: state.maxCombo,
        averageSatisfaction: score.averageSatisfaction,
        discoveries,
        seenMenuStages: receipt.seenMenuStages,
      },
    });
    if (commitError) {
      console.error("Shift commit failed", commitError.code);
      return json({ error: "SETTLEMENT_FAILED" }, 500);
    }
