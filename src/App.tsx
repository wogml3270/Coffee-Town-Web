...
export const App = () => { const screen = useGame(({ screen }) => screen); useEffect(() => { const click = (event: MouseEvent) => { if ((event.target as Element | null)?.closest("button:not(:disabled)")) soundPlayer.playUi(); }; document.addEventListener("click", click); return () => document.removeEventListener("click", click); }, []); return screen === "title" ? <Title/> : screen === "result" ? <Result/> : screen === "upgrade" ? <Upgrade/> : <Shift/>; };
