...
export const useGame = create<GameStore>()(persist((set, get) => ({
  ...
  bankGold: 0, upgrades: defaultUpgrades, selectedStage: 1, unlockedStage: 1,
...
}), { name: "coffee-town-local-progress", partialize: ({ bankGold, upgrades, selectedStage, unlockedStage }) => ({ bankGold, upgrades, selectedStage, unlockedStage }), merge: (persisted, current) => { const saved = persisted as Partial<GameStore>; return { ...current, ...saved, upgrades: { ...current.upgrades, ...saved.upgrades } }; } }));
