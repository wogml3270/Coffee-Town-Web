...
export const applyShiftAction = (
  ...
  switch (action.kind) {
    ...
    case "fridge":
      ...
      next = takeFridgeIngredient(state, action.target as ItemId);
      break;
    case "water":
...
export const verifyShift = (session: ShiftSession, receipt: ShiftReceipt, wallSeconds: number) => {
  if (receipt.version !== shiftProtocolVersion || receipt.sessionId !== session.id)
