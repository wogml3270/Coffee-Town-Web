...
const read = (userId: string): readonly ShiftReceipt[] => {
  ...
  if (
    !Array.isArray(value) ||
    value.some((entry) => !entry || typeof entry.sessionId !== "string" || !Array.isArray(entry.actions))
  )
    throw new Error("INVALID_PENDING_PROGRESS");
...
export const receiptStorage: ReceiptStorage = {
  ...
  put: (userId, receipt) => {
    ...
    if (!entries.some(({ sessionId }) => sessionId === receipt.sessionId))
      localStorage.setItem(prefix + userId, JSON.stringify([...entries, receipt]));
  },
